...
class _AnimatedTemperatureState
    extends AnimatedWidgetBaseState<AnimatedTemperature> {
  ...
  double temperatureToRelative(double temperature) {
    final currentScale = temperatureScale[widget.model.unit],
  ...
  void forEachTween(TweenVisitor<dynamic> visitor) {
    _temperatureRelative = visitor(
      ...
      temperatureToRelative(widget.model.temperature as double),
      (value) => Tween<double>(begin: value as double),
    ) as Tween<double>;

    _lowRelative = visitor(
      _lowRelative,
      temperatureToRelative(widget.model.low as double),
      (value) => Tween<double>(begin: value as double),
    ) as Tween<double>;

    _highRelative = visitor(
      _highRelative,
      temperatureToRelative(widget.model.high as double),
      (value) => Tween<double>(begin: value as double),
    ) as Tween<double>;
  }
...
class RenderTemperature
  ...
  RenderTemperature({
    ...
    required double temperatureRelative,
    required double lowRelative,
    required double highRelative,
    required Color textColor,
    required Color tubeColor,
    required Color mountColor,
    required Color backgroundPrimaryColor,
    required Color backgroundSecondaryColor,
    required Color borderColor,
    required Color bradColor,
    required Color bradHighlightColor,
    required Color temperatureColor,
    required Color maxTemperatureColor,
    required Color minTemperatureColor,
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    // so this is more convenient :)
    () {
      final w = bradRadius * 1.33,
          start = mount.startOffset(dx: size.width / 2),
          end = mount.endOffset(dx: size.width / 2);
      mountPath = Path()
    ...
    () {
      ...
      final startRect = brackets.startOffset(dx: dx) & bracketSize,
          startPath = Path()..addRect(startRect),
          startPaint = Paint()
            ..shader = bracketGradient.createShader(startRect);
      ...
      final endRect = brackets.endOffset(dx: dx) & bracketSize,
          endPath = Path()..addRect(endRect),
          endPaint = Paint()..shader = bracketGradient.createShader(endRect);
  ...
  void _drawLines(Canvas canvas, Line1d constraints) {
    ...
    final majorValue = _unit == TemperatureUnit.fahrenheit ? 20 : 10,
    ...
    final minMax = temperatureScale[_unit],
        min = minMax![0],
        max = minMax[1],
        difference = constraints.extent / max.difference(min) * minorValue;
