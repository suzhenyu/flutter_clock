import 'package:barbar_clock/const/const.dart';
import 'package:barbar_clock/model/display_bar.dart';
...
class DigitsCollection {
  ...
  List<DisplayBar> getTime(
    ...
    assert(oldHour.length == 2);
    assert(newHour.length == 2);
    assert(oldMinute.length == 2);
    assert(newMinute.length == 2);
  ...
  List<DisplayBar> _getDigitTransition(
    ...
    assert(digitFrom.isNotEmpty);
    assert(digitTo.isNotEmpty);
    assert(progress >= 0 && progress <= 1);

    if (progress == 0.0) return digitFrom;
    if (progress == 1.0) return digitTo;

    final result = <DisplayBar>[];
  ...
  DisplayBar _createBarFrom(DisplayBar fromBar,
