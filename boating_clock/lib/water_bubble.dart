...
class Point {
  double aX, aY, dX, dY, dA, x, y, r, aA;
  double aR;
  ...
  static List<Point> randomPoints(double maxX) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      points.add(Point(
          ...
          aA: originAAlpha,
          aR: 0.99,
          color: const Color(0xFFc3cddc).withOpacity(originAAlpha)));
    }
    return points;
  }
...
class FloatingPointView extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Point.remained.forEach((c) {
      if (c.isEnable) {
        _actionRemained(c);
        canvas.drawCircle(Offset(c.x, c.y), c.r, pointPainter..color = c.color);
      }
    });
...
//      });
    ...
    _points.forEach((c) {
      ...
      if (newPoint.isEnable) {
        Rect rect = Offset(newPoint.x - 5, newPoint.y - 5) & Size(10, 10);

        pointPainter.shader = RadialGradient(
          radius: 0.9,
          colors: [
            Colors.white.withOpacity(newPoint.color.opacity),
            newPoint.color
          ],
        ).createShader(rect);
...
//        newPoint.isEnable = true;
  ...
  Point _fixPointByRadius(Point point) {
    if (radius >= (waterRadians) && radius < math.pi) {
      ...
      point.y =
          (originY - paddingTop) * math.Random().nextInt(100).toDouble() / 100;
      var tanX = (point.y + paddingTop) / math.tan((radius - math.pi / 2));
      double maxX =
          math.max(0, _containerSize.width / 2 - tanX + paddingHorizon);
      if (point.x > maxX) {
//        point.isEnable = false;
      ...
      point.y =
          (originY - paddingTop) * math.Random().nextInt(120).toDouble() / 100;
      double maxX = math.tan(radius - math.pi) * (point.y + paddingTop) +
...
//        point.isEnable = false;
      ...
      if (point.x >= (_containerSize.width / 2 + paddingHorizon - 100)) {
        if (point.y >= 0) {
          point.y = 0;
          point.x =
              (handLength + _containerSize.width / 2 + paddingHorizon) - 50;
        }
...
//    }
    ...
    var opacity =
        math.max(0.8 * (radius - math.pi / 2) / (3 * math.pi / 2), 0.0);
//
    if (opacity >= 0.8) {
      opacity = 0.1;
      _point.dA = 0.1;
