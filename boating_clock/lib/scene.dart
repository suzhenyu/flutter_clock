...
import 'package:boating_clock/configs.dart';
import 'dart:async';

import 'package:boating_clock/drawn_hand.dart';
import 'package:boating_clock/water_bubble.dart';
...
class Scence extends StatefulWidget {
  const Scence({super.key});

  @override
  State<Scence> createState() => ScenceState();
}
...
class ScenceState extends State<Scence> {
  ...
  Point _sunPosition =
      Point(x: _sunSize / 2, y: _sunSize / 2, dX: 1, dY: 1, aX: 1, aY: 1);
  ...
  void _updateTime() {
    ...
    if (_controller.positions.isNotEmpty &&
        _controller.position.minScrollExtent != null) {
      _controller.jumpTo(currentHour * _textHeight +
          minute * _textHeight / 2 -
          bounus +
          _textHeight -
...
//        padding: EdgeInsets.all(100),
            decoration: BoxDecoration(
              gradient: RadialGradient(
                ...
                colors: _isNight
                    ? <Color>[
                        Color(0xFF373938).withOpacity(_sunOpacity),
                        Color(0xFF373938).withOpacity(_sunOpacity)
                      ]
            ...
            child: Stack(
              ...
              children: [
                _isNight
                    ? Opacity(
                        opacity: 0.3 +
                            0.3 *
                                math
                                    .sin(_now.millisecond * radiansPerSecond)
                                    .abs(),
                        child: Center(
                            child: Transform.translate(
                                offset: _caculateSunPosition(),
                                child: Transform.rotate(
                                    angle: _now.millisecond * radiansPerSecond,
                                    child: Image.asset(
                    ...
                    : Align(
                        ...
                        child: Opacity(
                          opacity: _sunOpacity,
                          child: Transform.translate(
                            offset: Offset(0, -_sunBottomPadding),
                                  ...
                                  (0.9 +
                                      0.1 *
                                          math
                                              ...
                                              .abs()),
                              child: Transform.rotate(
                                  angle: _now.second * radiansPerRealSecond,
                                  child: Image.asset(
                                    'assets/images/sun.png',
                                    fit: BoxFit.fill,
                                    width: _sunSize,
                ...
                Transform.translate(
                  offset: Offset(_currentTrackPadding, -transformY),
                  child: Transform.rotate(
                    angle: -math.pi / 8 +
                        math.sin(math.pi * _now.millisecond / 1000) *
                            math.pi /
                            4,
                    child: Container(
...
//                          ),
                ...
                Padding(
                  padding: EdgeInsets.only(
                      left: 40.0 + MediaQuery.of(context).padding.left),
                  child: ListView.builder(
                      ...
                      itemBuilder: (ctx, idx) {
                        return Container(
                            ...
                            child: Text(
                              (idx % 24).toString(),
                              textScaleFactor: idx == currentHour ? 1.3 : 1,
                              style: Theme.of(context)
                                  .textTheme
                                  .titleLarge
                                  ?.copyWith(
                                      color: idx > currentHour
                                          ? Color(0xFF102343).withOpacity(0.5)
                                          : Colors.white.withOpacity(
                                              idx == currentHour ? 1 : 0.5)),
                            ));
                      }),
                ),
                WaveWidget(
                  config: CustomConfig(
                    gradients: [
                      [
                        Color(0xFF224c7e).withOpacity(0.7),
                        Color(0xFF224c7e).withOpacity(0.3)
                      ],
                      [
                        Color(0xFF224c7e).withOpacity(0.3),
                        Color(0xFF224c7e).withOpacity(0.6)
                      ],
                    ],
                    durations: [5200, 5000],
...
//                          ),
  ...
  double get _trackDistance =>
      _trackMargin / 2 +
      _trackLength * (60 - currentMinutes) / 60 -
      _trackLength / 2;
  static const double _trackMargin = 120;
  double get _transformY =>
      -math.sin(2 * math.pi * _now.millisecond / 1000) * 20;
  bool get _isNight => currentHour < 6 || currentHour > 18;
  double get _sunBottomPadding {
    double canvasHeight =
        MediaQuery.of(context).size.height / 2 - _sunSize - 20;
    if (currentHour > 14) {
  ...
  Offset _caculateSunPosition() {
    ...
    _sunPosition.dY = _sunPosition.aY *
        math.sin(_now.millisecond * radiansPerSecond * 0.7) *
        _sunPosition.dX;
    _sunPosition.y += _sunPosition.dY;

    if (_sunPosition.y >=
        MediaQuery.of(context).size.height / 2 - _sunSize / 2) {
    ...
    if (_sunPosition.x >=
        MediaQuery.of(context).size.width / 2 - _sunSize / 2) {
    ...
    double minshipArea =
        MediaQuery.of(context).size.width - _trackDistance - handLength;
    double maxshipArea =
        MediaQuery.of(context).size.width - _trackDistance + handLength;
    double sunPosition = MediaQuery.of(context).size.width - _sunPosition.x;
    double handY = handLength - (12 - _transformY);

    if (sunPosition >= minshipArea &&
        sunPosition <= maxshipArea &&
        _sunPosition.y < handY + 20 &&
        _sunPosition.isEnable) {
